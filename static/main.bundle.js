...
var MainPageComponent = (function () {
    function MainPageComponent(httpService, dialog, progressService, router, dataService, variableService) {
        ...
        this.isExtendedBoardConnected = false;
        this.elements = {
            settings: {
    ...
    MainPageComponent.prototype.ngOnInit = function () {
        ...
        setTimeout(function () {
            _this.httpService.get('extended_board_connection', true).subscribe(function (data) {
                _this.isExtendedBoardConnected = data['is_connected'];
            }, function (err) {
                console.warn('could not get the extended board connection status: ', err);
            });
        }, 10000);
    };
    ...
    LastPrintsComponent.prototype.getLastPrintedFiles = function () {
        ...
        this.httpService.get('last_prints', true).subscribe(function (data) {
            data.forEach(function (row) {
                ...
                row['is_finished'] = (row['is_finished'].toLowerCase() === 'yes' ? 'Yes' : 'No');
            });
